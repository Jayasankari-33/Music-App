...
function Playlist() {
  ...
  const playAllSongs = () => {
    ...
    if (!playlist.length) return; // Exit if playlist is empty
    ...
    if (currentSongAudio.paused) {
      ...
      currentSongAudio.addEventListener('ended', function playNextSong() {
        const currentIndex = playlist.findIndex(item => `audio-${item.id}` === currentSongAudio.id);
        ...
        if (nextSongAudio) {
          // Play the next song
          nextSongAudio.play();
          currentSongAudio.removeEventListener('ended', playNextSong); // Remove listener from the previous song
        }
      });
      setIsPlaying(true)
